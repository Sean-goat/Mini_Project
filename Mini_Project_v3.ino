...
int left_tableRef = 560;  // Color calibrate
int left_wallRef = 180;   // Color calibrate

int mid_tableRef = 560;  // Color calibrate
int mid_wallRef = 180;   // Color calibrate

int right_tableRef = 560;  // Color calibrate
int right_wallRef = 180;   // Color calibrate

// === Speeds ===
int forwardSpeed = 170;      // straight drive speed
int smallForwardSpeed = 20;  // speed while truning
int turnSpeed = 100;
...
bool isWall(uint16_t val, int tableRef, int wallRef) {
  return abs((int)val - wallRef) < abs((int)val - tableRef);  // returns the closest value
}

// findes entry to maze
void driveStraightUntilWall() {
  while (true) {
    lineSensors.read(lineSensorValues, QTR_EMITTERS_ON);
    bool midIsWall = isWall(lineSensorValues[1], mid_tableRef, mid_wallRef);  // untill the middel sensor findes the maze, then the code starts
    ...
    motors.setSpeeds(forwardSpeed, forwardSpeed);  // drives forward untill wall i found
    delay(5);
  }
}
